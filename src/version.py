...
import json
from unit_result import remove_whitespace,read_patch
...
def split_diff_lines_to_json(lines):
    #TODO: 合并一个文件的patch中修改同一个hunk的部分
    hunk_start_indices = []
    last_hunk_header = ""
    for i, line in enumerate(lines):
        if line.startswith('@@'):
            hunk_header = read_patch(line)
            if last_hunk_header != hunk_header:
                last_hunk_header = hunk_header
    ...
    file_header = []
    for i in range(hunk_start_indices[0]):
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(versions)-1)):
        result = subprocess.run(["git","diff","-U",versions[i],versions[i+1]],cwd="./v8", stdout=subprocess.PIPE)
        output = result.stdout.decode('utf-8', errors='ignore')
        patch = output.split("\n")
        ...
        for line in patch:
            ...
            if filename != []:
                if filtered_patch != []:
                    hunks = split_diff_lines_to_json(filtered_patch)
